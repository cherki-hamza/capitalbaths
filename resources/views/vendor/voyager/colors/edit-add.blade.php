                            ...
                            @foreach ($dataTypeRows as $row)
                                ...
                                <div class="form-group @if ($row->type == 'hidden') hidden @endif col-md-{{ $display_options->width ?? 12 }} {{ $errors->has($row->field) ? 'has-error' : '' }}"
                                    ...
                                    <div class="form-group">
                                        @php
                                            // Fetching the image value from the model or defaulting to null
                                            $image = $dataTypeContent->{$row->field};
                                        @endphp

                                        {{-- Display current image if exists --}}
                                        @if($image)
                                            <img src="{{ Voyager::image($image) }}" alt="Current Image" style="width: 200px; height: auto;">
                                        @else
                                            <img src="{{ asset('assets/img/icons/no_image.jpg') }}" alt="Default Image" style="width: 200px; height: auto;">
                                        @endif

                                        {{-- Image input for uploading new image --}}
                                        <input type="file" name="{{ $row->field }}" accept="image/*">
                                    </div>
